...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-if="!isConnected" class="flex-1 flex flex-col items-center justify-center text-center">
      <div class="i-mdi-microsoft-onedrive w-lg h-lg text-info-normal"></div>
      <h3 class="text-xl font-semibold mt-md">Connect to OneDrive</h3>
      <p class="mt-2 text-border-normal max-w-sm">
        Access and attach files directly from your OneDrive account.
      </p>
      <button
        @click="connectAccount"
        class="mt-md flex items-center space-x-2 px-md py-3 bg-info-active text-white rounded-lg font-semibold hover:bg-info-normal"
      >
    ...
    <div v-else class="flex flex-col h-full">
      <div class="flex items-center justify-between mb-md">
        <p class="text-sm text-border-normal">
          Connected as: <span class="font-semibold text-background-normal">user@outlook.com</span>
        </p>
        <button
